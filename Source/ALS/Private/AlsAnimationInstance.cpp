...
void UAlsAnimationInstance::RefreshView(const float DeltaTime)
{
	...
	{
		// View angles relative to the character must be measured in gravity space under custom gravity.
		if (UAlsGravityUtility::HasCustomGravity(Character))
		{
			...
			const FQuat GravitySpaceViewRotation = WorldToGravityTransform * ViewState.RotationWorldSpace.Quaternion();
			const FQuat GravitySpaceCharacterRotation = WorldToGravityTransform * LocomotionState.RotationQuaternionWorldSpace;
...
void UAlsAnimationInstance::RefreshHead()
{
	...
	if (RotationMode == AlsRotationModeTags::VelocityDirection)
	{
		...
		// Character rotation yaw must be in the same coordinate space as the input and target
		// yaw angles, which is gravity space under custom gravity.
		float CharacterRotationYaw;
		if (UAlsGravityUtility::HasCustomGravity(Character))
		{
			const FQuat WorldToGravityTransform = UAlsGravityUtility::GetWorldToGravityTransform(Character);
			const FQuat GravitySpaceRotation = WorldToGravityTransform * LocomotionState.RotationQuaternionWorldSpace;
			CharacterRotationYaw = UE_REAL_TO_FLOAT(GravitySpaceRotation.Rotator().Yaw);
		}
		else
		{
			CharacterRotationYaw = UE_REAL_TO_FLOAT(LocomotionState.RotationWorldSpace.Yaw);
		}
...
void UAlsAnimationInstance::RefreshLocomotionOnGameThread()
{
	...
	FQuat PreviousRotationQuaternion{LocomotionState.RotationQuaternionWorldSpace};
	...
	// Yaw velocity must be measured in gravity space under custom gravity.
	if (bCanCalculateRateOfChange)
	{
		if (UAlsGravityUtility::HasCustomGravity(Character))
		{
			...
			const FQuat GravitySpaceCurrentRotation = WorldToGravityTransform * LocomotionState.RotationQuaternionWorldSpace;
			...
			LocomotionState.YawVelocityWorldSpace = FMath::UnwindDegrees(CurrentGravityYaw - PreviousGravityYaw) / DeltaTime;
		}
		else
		{
			LocomotionState.YawVelocityWorldSpace = FMath::UnwindDegrees(UE_REAL_TO_FLOAT(
				LocomotionState.RotationWorldSpace.Yaw - PreviousYawAngle)) / DeltaTime;
		}
	}
	else
	{
		LocomotionState.YawVelocityWorldSpace = 0.0f;
	}
...
{
	// Character-relative space is already gravity-aligned because the character itself is oriented
	// to gravity, so no additional gravity-space transform is needed here.
...
void UAlsAnimationInstance::RefreshVelocityBlend()
{
	...
	{
		// Under custom gravity the relative velocity's Z axis is the gravity axis; keep directional
		// blending horizontal by excluding it from normalization.
		const bool bInGravitySpace = UAlsGravityUtility::HasCustomGravity(Character);

		TargetVelocityBlend = VelocityDirection /
		                      (FMath::Abs(VelocityDirection.X) + FMath::Abs(VelocityDirection.Y) +
		                       (bInGravitySpace ? 0.0f : FMath::Abs(VelocityDirection.Z)));
	}
...
void UAlsAnimationInstance::RefreshGroundedMovement()
{
	...
	// Velocity and view yaw angles must be compared in the same coordinate space, which is
	// gravity space under custom gravity.
	float VelocityYawAngleViewSpace;

	if (UAlsGravityUtility::HasCustomGravity(Character))
	{
		const FQuat WorldToGravityTransform = UAlsGravityUtility::GetWorldToGravityTransform(Character);
		const FVector3f GravitySpaceVelocity = FVector3f{WorldToGravityTransform.RotateVector(LocomotionState.VelocityWorldSpace)};
		...
		VelocityYawAngleViewSpace = FMath::UnwindDegrees(GravitySpaceVelocityYawAngle - GravitySpaceViewYaw);
	}
	else
	{
		VelocityYawAngleViewSpace = FMath::UnwindDegrees(UE_REAL_TO_FLOAT(
			LocomotionState.VelocityYawAngleWorldSpace - ViewState.RotationWorldSpace.Yaw));
	}
